...
const PHASE_04_1_MIGRATION = '20260713000000_phase_04_1_pim_activation';
...
async function inspect() {
  ...
  try {
    ...
    if (!identity || identity.database !== EXPECTED_PIM_TEST_DATABASE || identity.role !== EXPECTED_PIM_TEST_USER || identity.schema !== 'public') {
      throw new Error('Connected database identity does not match the isolated PIM test target.');
    }
    ...
    if (expectation === 'empty' && publicObjects.length !== 0) {
      throw new Error('Expected a pristine isolated PIM test database before migration; public schema objects already exist.');
    }
    ...
    if (expectation === 'migrated') {
      ...
      `;
      const phaseMigration = migrations.find((migration) => migration.name === PHASE_04_1_MIGRATION);
      if (!phaseMigration || !phaseMigration.checksum || !phaseMigration.finishedAt || phaseMigration.rolledBackAt) {
        throw new Error('Migration verification failed; the Phase 04.1 baseline is not a completed, checksum-bearing Prisma migration.');
      }
    }

    console.log(`PIM test database verified: database=${identity.database}, role=${identity.role}, schema=${identity.schema}, tables=${tableNames.length}.`);
  } finally {
...
inspect().catch((error) => {
  console.error(error instanceof Error ? error.message : 'PIM test database inspection failed.');
  process.exitCode = 1;
});
