...
export const PIM_BENCHMARK_MIGRATION = '20260713000000_phase_04_1_pim_activation';
...
export function validatePimBenchmarkApiBaseUrl(value) {
  ...
  return {
    ok: false,
    errors: [`PIM_BENCHMARK_API_BASE_URL must be exactly "${PIM_BENCHMARK_API_BASE_URL}".`],
  };
}

export function resolvePimBenchmarkApiOptions(environment = process.env) {
  const apiTarget = validatePimBenchmarkApiBaseUrl(environment.PIM_BENCHMARK_API_BASE_URL);
  ...
  if (p95ThresholdMs > requestTimeoutMs) {
    throw new Error('PIM_BENCHMARK_API_P95_MS must be less than or equal to PIM_BENCHMARK_API_TIMEOUT_MS.');
  }
...
export function validatePimBenchmarkEnvironment(environment = process.env) {
  ...
  if (typeof runId !== 'string' || !/^[a-z0-9](?:[a-z0-9-]{6,38}[a-z0-9])?$/.test(runId)) {
    errors.push('PIM_BENCHMARK_RUN_ID must be 8-40 lowercase letters, digits, or hyphens and cannot start or end with a hyphen.');
  }

  if (environment.DATABASE_URL && environment.DATABASE_URL !== environment.PIM_TEST_DATABASE_URL) {
    errors.push('DATABASE_URL must be unset or exactly match PIM_TEST_DATABASE_URL for this benchmark.');
  }
  ...
  try {
    ...
    errors.push(error instanceof Error ? error.message : 'PIM_BENCHMARK_BATCH_SIZE is invalid.');
  }
  ...
  try {
    ...
    errors.push(error instanceof Error ? error.message : 'PIM_BENCHMARK_EXPLAIN_RUNS is invalid.');
  }

  try {
    resolvePimBenchmarkApiOptions(environment);
  } catch (error) {
    errors.push(error instanceof Error ? error.message : 'PIM benchmark API options are invalid.');
  }
...
export function parseBenchmarkArguments(argumentsList = process.argv.slice(2)) {
  if (argumentsList.length === 1 && argumentsList[0] === '--help') return { help: true, execute: false };
  if (argumentsList.length === 1 && argumentsList[0] === '--execute') return { help: false, execute: true };
  throw new Error('Use exactly --execute after setting the guarded isolated benchmark environment. Use --help for details.');
}
...
async function assertIsolatedMigratedDatabase(prisma) {
  ...
  if (
    ...
    throw new Error('Connected database identity does not match the isolated PIM benchmark target.');
  }
  ...
  `;
  const tableNames = new Set(tableRows.map((row) => stringValue(recordValue(row)?.name)).filter(Boolean));
  const missingTables = REQUIRED_TABLES.filter((name) => !tableNames.has(name));
  if (missingTables.length > 0) {
    throw new Error(`Required Phase 04.1 tables are missing from the isolated database: ${missingTables.join(', ')}.`);
  }
  ...
  if (!migration || !migration.finishedAt || migration.rolledBackAt) {
    throw new Error('The completed Phase 04.1 migration is required before benchmark data can be seeded.');
  }

  const versionRows = await prisma.$queryRaw`SHOW server_version`;
  const version = stringValue(recordValue(versionRows[0])?.server_version) ?? 'unknown';
  console.log(`PIM benchmark target verified: database=${EXPECTED_PIM_TEST_DATABASE}, role=${EXPECTED_PIM_TEST_USER}, schema=public, PostgreSQL=${version}.`);
}
...
async function assertUnusedRun(prisma, context) {
  const [products, categories, variants, skus] = await prisma.$transaction([
    ...
    prisma.catalogVariant.count({ where: { sku: { startsWith: `PIMBENCH-${context.runId.toUpperCase()}-` } } }),
    prisma.productSku.count({ where: { code: { startsWith: `PIMBENCH-${context.runId.toUpperCase()}-` } } }),
  ]);

  if (products > 0 || categories > 0 || variants > 0 || skus > 0) {
    throw new Error('PIM_BENCHMARK_RUN_ID has already created fixture records or a partial run. Choose a new run id; this harness never mutates existing fixtures.');
  }
}
...
async function createBenchmarkCategory(prisma, context) {
  await prisma.catalogCategory.create({
    data: {
      ...
      description: 'Retained Phase 04.1 isolated performance benchmark fixture.',
      sortOrder: 9_999,
      isActive: true,
    },
  });
...
async function seedToScale(prisma, context, currentCount, targetCount, batchSize) {
  const startedAt = performance.now();
  let inserted = 0;

  for (let ordinal = currentCount + 1; ordinal <= targetCount; ordinal += batchSize) {
    ...
    if (completed === targetCount || completed % 10_000 === 0) {
      console.log(`PIM benchmark fixture progress: products=${completed}/${targetCount}.`);
    }
  }

  const persistedCount = await prisma.catalogProduct.count({ where: { searchText: context.marker } });
  if (persistedCount !== targetCount) {
    throw new Error(`Benchmark fixture count mismatch: expected ${targetCount}, found ${persistedCount}. Existing records were left untouched.`);
  }
...
function parseExplainDocument(rows) {
  ...
  const document = Array.isArray(parsed) ? recordValue(parsed[0]) : recordValue(parsed);
...
function summarizeRuns(runs) {
  const execution = runs.map((run) => run.executionMs).filter((value) => value !== null);
  const planning = runs.map((run) => run.planningMs).filter((value) => value !== null);
  if (execution.length === 0 || planning.length === 0) {
    throw new Error('EXPLAIN output did not include planning and execution timings.');
  }
  const last = runs.at(-1);
  return {
    runs: runs.length,
    executionMs: { p50: percentile(execution, 0.5), p95: percentile(execution, 0.95) },
    planningMs: { p50: percentile(planning, 0.5), p95: percentile(planning, 0.95) },
...
export function publicApiBenchmarkPaths(categorySlug, productSlug) {
  ...
  return [
    ...
    { path: 'public-category-filtered-listing', requestPath: `${listingPath}&category=${encodeURIComponent(categorySlug)}` },
    { path: 'public-detail', requestPath: `/api/products/${encodeURIComponent(productSlug)}` },
...
export function summarizePublicApiBody(bodyText, byteLength, contentType) {
  const summary = {
    bytes: byteLength,
    contentType: contentType?.toLowerCase().startsWith('application/json') ? 'application/json' : 'other',
  ...
  try {
    ...
    summary.json = {
      ...
      data: dataRecord
        ? {
          type: 'object',
          itemCount: Array.isArray(dataRecord.items) ? dataRecord.items.length : null,
          total: numberValue(dataRecord.total),
        }
        : { type: jsonType(data), itemCount: Array.isArray(data) ? data.length : null, total: null },
    };
  } catch {
...
async function readBoundedResponseBody(response) {
  ...
  if (Number.isFinite(contentLength) && contentLength > MAX_API_RESPONSE_BYTES) {
    throw new Error('Public API response exceeds the benchmark body-size limit.');
  }
...
async function measurePublicApiRequest(apiOptions, endpoint) {
  ...
  if (url.origin !== PIM_BENCHMARK_API_BASE_URL || url.protocol !== 'http:') {
    throw new Error('Public API benchmark URL escaped the guarded local target.');
  }
  ...
  try {
    ...
    throw new Error(`Public API benchmark request failed or timed out for ${endpoint.path}.`);
  }
  ...
  return {
    ...
    body: summarizePublicApiBody(body.text, body.bytes.byteLength, response.headers.get('content-type')),
  };
}
...
async function runPublicApiPath(apiOptions, endpoint) {
  ...
  return {
    ...
    status: { expected: 200, counts: statusCounts, allExpected: samples.every((sample) => sample.status === 200) },
  };
}

async function runPublicApiBenchmark(apiOptions, context) {
  const paths = publicApiBenchmarkPaths(context.categorySlug, context.productSlug(1));
  const results = [];
  for (const path of paths) results.push(await runPublicApiPath(apiOptions, path));
  return results;
}
...
function apiBenchmarkViolations(scale, paths, p95ThresholdMs) {
  return paths.flatMap((path) => {
    const violations = [];
    if (!path.status.allExpected) violations.push(`${scale}:${path.path} returned non-200 status.`);
    if (path.latencyMs.p95 > p95ThresholdMs) {
      violations.push(`${scale}:${path.path} p95=${path.latencyMs.p95}ms exceeds ${p95ThresholdMs}ms.`);
    }
    return violations;
  });
}
...
async function runScaleBenchmark(prisma, context, scale, explainRuns, seedResult, apiOptions) {
  const paths = await Promise.all([
    runExplainPath('public-listing', () => explainPublicListing(prisma), explainRuns),
    runExplainPath('public-category-filtered-listing', () => explainCategoryFilteredListing(prisma, context.categoryId), explainRuns),
    runExplainPath('public-detail', () => explainPublicDetail(prisma, context.productSlug(1)), explainRuns),
    runExplainPath('public-category-list', () => explainPublicCategories(prisma), explainRuns),
    runExplainPath('import-sku-validation', () => explainImportSkuValidation(prisma, context.skuCode(scale)), explainRuns),
    runExplainPath('workflow-history', () => explainWorkflowHistory(prisma, context.productId(1)), explainRuns),
  ]);
  const apiPaths = await runPublicApiBenchmark(apiOptions, context);
  const apiViolations = apiBenchmarkViolations(scale, apiPaths, apiOptions.p95ThresholdMs);
...
async function executeBenchmark(environment) {
  ...
  const batchSize = parseBoundedInteger(environment.PIM_BENCHMARK_BATCH_SIZE, DEFAULT_BATCH_SIZE, MIN_BATCH_SIZE, MAX_BATCH_SIZE, 'PIM_BENCHMARK_BATCH_SIZE');
  const explainRuns = parseBoundedInteger(environment.PIM_BENCHMARK_EXPLAIN_RUNS, DEFAULT_EXPLAIN_RUNS, MIN_EXPLAIN_RUNS, MAX_EXPLAIN_RUNS, 'PIM_BENCHMARK_EXPLAIN_RUNS');
  ...
  try {
    ...
    for (const scale of PIM_BENCHMARK_SCALES) {
      const seedResult = await seedToScale(prisma, context, seeded, scale, batchSize);
      seeded = scale;
      if (!workflowFixtureCreated) {
        await createWorkflowHistoryFixture(prisma, context);
        workflowFixtureCreated = true;
      }
      apiViolations.push(...await runScaleBenchmark(prisma, context, scale, explainRuns, seedResult, apiOptions));
    }
    if (apiViolations.length > 0) {
      throw new Error(`PIM benchmark API quality gate failed: ${apiViolations.join(' ')}`);
    }
  } finally {
...
if (isDirectExecution()) {
  try {
    ...
    if (argumentsResult.help) {
      ...
      if (!preflight.ok) {
        throw new Error(`PIM benchmark environment preflight failed: ${preflight.errors.join(' ')}`);
      }
      await executeBenchmark(process.env);
    }
  } catch (error) {
    console.error(error instanceof Error ? error.message : 'PIM benchmark execution failed.');
    process.exitCode = 1;
  }
}
