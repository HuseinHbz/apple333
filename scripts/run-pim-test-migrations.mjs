...
function main() {
  ...
  const environment = { ...process.env, DATABASE_URL: process.env.PIM_TEST_DATABASE_URL };
  const inspectScript = resolve('scripts/inspect-pim-test-database.mjs');
...
try {
  ...
  console.error(error instanceof Error ? error.message : 'PIM migration execution failed.');
  process.exitCode = 1;
}
