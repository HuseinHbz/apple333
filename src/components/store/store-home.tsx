...
export function StoreHome() {
  const categories = useQuery({
    ...
    queryFn: () => storeApi<readonly PublicCategoryDto[]>('/api/store/categories'),
  });
    ...
    <main>
      ...
      <section className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            <p className="text-xs font-bold tracking-[0.18em] text-zinc-500">CATALOG</p>
      ...
      <section className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
        <div className="rounded-3xl border border-zinc-200 bg-white p-6 sm:p-8">
          ...
          <div className="mt-6 flex flex-wrap gap-3">
            ...
            {categories.data?.map((category) => <Link key={category.id} href={`/products?category=${encodeURIComponent(category.slug)}`} className="rounded-full border border-zinc-200 px-4 py-2 text-sm font-semibold transition hover:border-zinc-950 hover:bg-zinc-950 hover:text-white">{category.name}</Link>)}
          </div>
        </div>
      </section>
    </main>
