...
describe("PIM performance benchmark harness", () => {
  ...
  it("accepts only the explicit guarded isolated benchmark environment", () => {
    expect(validatePimBenchmarkEnvironment(validEnvironment())).toEqual({
      ok: true,
      errors: [],
    });
  });
  ...
  it("fails closed when the large fixture opt-in, run identifier, or ambient database target is unsafe", () => {
    const noOptIn = validatePimBenchmarkEnvironment({
      ...validEnvironment(),
      PIM_BENCHMARK_ALLOW_SEED: undefined,
    });
    expect(noOptIn.errors).toContain(
      'PIM_BENCHMARK_ALLOW_SEED must be exactly "1".',
    );

    const unsafeRunId = validatePimBenchmarkEnvironment({
      ...validEnvironment(),
      PIM_BENCHMARK_RUN_ID: "BAD",
    });
    expect(unsafeRunId.errors).toContain(
    ...
    const unsafeDatabaseUrl = validatePimBenchmarkEnvironment({
      ...validEnvironment(),
      DATABASE_URL:
        "postgresql://production:secret@127.0.0.1:5432/production?schema=public",
    });
    expect(unsafeDatabaseUrl.errors).toContain(
  ...
  it("requires an explicit --execute command and bounds runtime tuning values", () => {
    expect(parseBenchmarkArguments(["--execute"])).toEqual({
      help: false,
      execute: true,
    });
    expect(parseBenchmarkArguments(["--help"])).toEqual({
      help: true,
      execute: false,
    });
    ...
    expect(() => parseBoundedInteger("1001", 500, 100, 1_000, "batch")).toThrow(
      "batch must be between 100 and 1000.",
    );
  });

  it("allows only the explicit loopback API target and coherent bounded API thresholds", () => {
    expect(validatePimBenchmarkApiBaseUrl(PIM_BENCHMARK_API_BASE_URL)).toEqual({
      ok: true,
      errors: [],
    });
    expect(validatePimBenchmarkApiBaseUrl("http://localhost:3000").ok).toBe(
      false,
    );
    expect(validatePimBenchmarkApiBaseUrl("http://127.0.0.1:3000/").ok).toBe(
      false,
    );
    ...
    expect(() =>
      resolvePimBenchmarkApiOptions({
        ...validEnvironment(),
        PIM_BENCHMARK_QUERY_P95_MS: "9",
      }),
    ).toThrow("PIM_BENCHMARK_QUERY_P95_MS must be between 10 and 30000.");
  });
  ...
  it("uses five fixed, public GET paths and emits body metadata without raw response content", () => {
    ...
    expect(
      publicApiBenchmarkPaths("benchmark-category", "benchmark-product"),
    ).toEqual([
      {
        path: "public-listing",
        requestPath: "/api/products?page=1&pageSize=24&sort=newest",
      },
      {
        path: "public-category-filtered-listing",
        requestPath:
          "/api/products?page=1&pageSize=24&sort=newest&category=benchmark-category",
      },
  ...
  it("extracts non-sensitive timing and index evidence from a JSON EXPLAIN result", () => {
    ...
    expect(summary).toMatchObject({
      ...
      sharedHitBlocks: 12,
      indexes: [
        "Index Scan:CatalogProduct:CatalogProduct_status_publishedAt_idx",
      ],
    });
  });
});
